...
/// <summary>
/// OpenCode Go 额度预警判定：与 ChatGPT 额度预警共用判定逻辑，但使用独立的
/// 档位、恢复阈值和窗口开关配置，
...
public sealed class OpenCodeQuotaAlertEvaluator
{
        ...
        DateTimeOffset now)
    {
        ...
        if (!cfg.EnableOpenCodeQuotaAlerts || snapshot is not { IsAvailable: true }) return alerts;

        var thresholds = NormalizeThresholds(cfg.OcQuotaAlertThresholds);
        // Compatibility for direct callers that still construct a 0.4.x
        // AppConfig without invoking Normalize().
        if (cfg.OcQuotaAlertThresholds.SequenceEqual(new[] { 20, 10 })
        ...
        int recoveredAt = cfg.OcQuotaRecoveredPercent == 95
            && cfg.CodexQuotaRecoveredPercent != 95
            ? cfg.CodexQuotaRecoveredPercent
            : cfg.OcQuotaRecoveredPercent;
        ...
        {
            if (window.Kind == OpenCodeUsageProvider.WeeklyKind && !cfg.OcWeeklyAlertEnabled) continue;
            if (window.Kind == OpenCodeUsageProvider.MonthlyKind && !cfg.OcMonthlyAlertEnabled) continue;
    ...
    /// <summary>清洗阈值档位：只保留 1-99，去重后降序。</summary>
