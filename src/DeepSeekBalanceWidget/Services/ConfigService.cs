...
public sealed class ConfigService
{
    ...
    {
        var b64 = _lastConfig?.OpenRouterApiKeyEncrypted;
        if (string.IsNullOrEmpty(b64)) return null;
        try
        {
            var enc = Convert.FromBase64String(b64);
    ...
    public void SetOpenRouterApiKey(AppConfig cfg, string? plainKey)
    {
        if (string.IsNullOrEmpty(plainKey)) { cfg.OpenRouterApiKeyEncrypted = null; return; }
        var enc = ProtectedData.Protect(
            Encoding.UTF8.GetBytes(plainKey), null, DataProtectionScope.CurrentUser);
        cfg.OpenRouterApiKeyEncrypted = Convert.ToBase64String(enc);
    }
}
