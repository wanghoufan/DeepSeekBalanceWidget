...
public partial class SettingsWindow : Window
{
    ...
    public SettingsWindow(ConfigService configService, AppConfig cfg)
    {
        ...
        OpenRouterKeyBox.Password = configService.GetOpenRouterApiKey() ?? "";
        ...
        switch (ThemeService.Normalize(cfg.ThemeMode))
        {
        ...
        EnableOpenRouterCheck.IsChecked = cfg.EnableOpenRouterMonitoring;
        EnableQuotaAlertCheck.IsChecked = cfg.EnableCodexQuotaAlerts;
        QuotaThresholdsBox.Text = cfg.GptQuotaAlertThresholds is { Count: > 0 } thresholds
            ? string.Join(", ", thresholds)
            : "20, 10";
        QuotaRecoveredBox.Text = Math.Clamp(cfg.GptQuotaRecoveredPercent, 1, 100).ToString();
        WeeklyAlertCheck.IsChecked = cfg.GptWeeklyAlertEnabled;
    ...
    private void ClearOpenRouter_Click(object sender, RoutedEventArgs e)
    {
        OpenRouterKeyBox.Password = "";
        _configService.SetOpenRouterApiKey(_cfg, null);
    }
    ...
    {
        string key = OpenRouterKeyBox.Password is { Length: > 0 } typed
            ? typed
            : _configService.GetOpenRouterApiKey() ?? "";
        if (string.IsNullOrWhiteSpace(key))
        {
            SetTestResult(OpenRouterTestResult, false, "✗ 请先填写 OpenRouter API Key");
            return;
        }
        ...
        await RunTestAsync(TestOpenRouterBtn, OpenRouterTestResult, async () =>
        {
            ...
            return snapshot.IsAvailable
                ? (true, $"✓ 连接成功 · {OpenRouterUsageFormatter.FormatRemaining(snapshot)}")
    ...
    private void Save_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(OpenRouterKeyBox.Password))
            _configService.SetOpenRouterApiKey(_cfg, OpenRouterKeyBox.Password);
        ...
        _cfg.ThemeMode = ThemeDarkRadio.IsChecked == true ? "Dark"
        ...
        _cfg.EnableOpenRouterMonitoring = EnableOpenRouterCheck.IsChecked == true;
        _cfg.EnableCodexQuotaAlerts = EnableQuotaAlertCheck.IsChecked == true;
        _cfg.GptQuotaAlertThresholds = quotaThresholds;
        _cfg.GptQuotaRecoveredPercent = recoveredPct;
        _cfg.GptWeeklyAlertEnabled = WeeklyAlertCheck.IsChecked == true;
        ...
        ThemeService.Apply(_cfg.ThemeMode);
        DialogResult = true;
        Close();
    }
