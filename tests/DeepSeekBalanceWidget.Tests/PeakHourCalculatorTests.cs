...
public class PeakHourCalculatorTests
{
    ...
    private static DateTime BeijingHourUtc(int hour)
        => BeijingDateHourUtc(2026, 7, 31, hour);

    private static DateTime BeijingDateHourUtc(int year, int month, int day, int hour)
        => TimeZoneInfo.ConvertTimeToUtc(
            new DateTime(year, month, day, hour, 0, 0, DateTimeKind.Unspecified), Beijing);
    ...
    [Fact]
    public void Peak_Default_Saturday_1000_NotPeak() =>
        Assert.False(PeakHourCalculator.IsPeak(BeijingDateHourUtc(2026, 8, 1, 10), null));

    [Fact]
    public void Peak_Default_Sunday_1500_NotPeak() =>
        Assert.False(PeakHourCalculator.IsPeak(BeijingDateHourUtc(2026, 8, 2, 15), null));
